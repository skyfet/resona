...
## Update 18 - Responsive Width Fix (Current Request)
- Fixed right-edge overflow by rebinding layout and canvas sizing to grid columns instead of viewport width.
- `#game` now uses `width: 100%` + `max-width: 930px` with centered block rendering.
- Added `min-width: 0` guard for all grid children to prevent intrinsic-content overflow.
- Hardened text wrapping in HUD/menu panels for long strings.
- Added medium breakpoint (`<=1180px`) to switch from 3-column layout to a 2-column + full-width HUD arrangement.
- Improved compact-screen behavior (`<=640px`) for HUD action buttons and panel spacing.

## TODO
- Run Playwright skill client after responsive changes and inspect artifacts.
- Validate page-level horizontal overflow at multiple viewport widths.

## Update 19 - Runtime Resize Override Fix
- Discovered additional root cause: `resizeCanvasDisplay()` in `src/gameplay/engine.js` forced `canvas.style.width` from viewport math, overriding responsive CSS and causing right overflow.
- Updated runtime resize logic to clamp non-fullscreen canvas width by parent container width (`canvas.parentElement.clientWidth`) with viewport guard.
- Fullscreen behavior remains viewport-based.

## TODO
- Re-run page-level overflow checks across multiple viewport widths.
- Re-run skill Playwright client and inspect latest screenshot/state artifacts.

## Update 20 - Responsive Validation Complete
- Ran page-level responsive checks after CSS + runtime fixes:
  - `output/responsive-check-v18c/overflow-report.json`
  - Viewports verified: 1100, 900, 768, 390.
  - Result: `scrollWidth === innerWidth` in all checks (before and after start).
- Manually inspected full-page screenshots:
  - `output/responsive-check-v18c/layout-before-390.png`
  - `output/responsive-check-v18c/layout-after-390.png`
  - `output/responsive-check-v18c/layout-after-900.png`
  - `output/responsive-check-v18c/layout-after-1100.png`
- Skill-required Playwright client rerun after final changes:
  - `output/web-game-responsive-v18-final/shot-0.png`
  - `output/web-game-responsive-v18-final/shot-1.png`
  - `output/web-game-responsive-v18-final/state-0.json`
  - `output/web-game-responsive-v18-final/state-1.json`
...
## Remaining TODO / Suggestions
- Optional: add an automated CI check that asserts `document.documentElement.scrollWidth <= window.innerWidth` on key breakpoints.

## Update 21 - Layout Positioning and Sizing Fix (Current Request)
- Reproduced a desktop layout regression after `Новая игра`:
  - when menu hides at wide widths (`>1180px`), grid auto-placement moved `.stage` into the first column,
  - canvas kept stale width and visually overflowed into neighboring area.
- Applied layout stability fixes in `styles.css`:
  - explicit desktop grid placement for all primary blocks:
    - `.menu { grid-column: 1; }`
    - `.stage { grid-column: 2; }`
    - `.hud { grid-column: 3; }`
- Applied runtime sizing fixes in `src/gameplay/engine.js`:
  - call `resizeCanvasDisplay()` immediately after hiding the menu in both:
    - `startGame()`
    - `applyCheckpointSnapshot()`
  - corrected canvas resize math to use parent **content width** (subtract stage paddings) instead of raw `clientWidth`, eliminating right-edge canvas spill.

## Validation (Update 21)
- Automated responsive audit before/after fixes:
  - initial repro artifacts: `output/responsive-audit-current/`
  - first fix pass: `output/responsive-audit-fix1/`
  - final pass after resize-math correction: `output/responsive-audit-fix2/`
- Final responsive verification (`output/responsive-audit-fix2/report.json`):
  - widths checked: `1280, 1180, 1100, 900, 320`
  - no horizontal overflow,
  - no stage/hud overlap,
  - canvas fully fits inside stage at all checked widths.
- Skill-required Playwright client rerun:
  - `output/web-game-layout-fix-v1/shot-0.png`
  - `output/web-game-layout-fix-v1/shot-1.png`
  - `output/web-game-layout-fix-v1/state-0.json`
  - `output/web-game-layout-fix-v1/state-1.json`
